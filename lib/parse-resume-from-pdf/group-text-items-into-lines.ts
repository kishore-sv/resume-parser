import { BULLET_POINTS } from "../parse-resume-from-pdf/extract-resume-from-sections/lib/bullet-points.js";

import type { Line, Lines, TextItems } from "../parse-resume-from-pdf/extract-resume-from-sections/types.js";
/**
...
const getTypicalCharWidth = (textItems: TextItems): number => {
  ...
  textItems = textItems.filter((item:any) => item.text.trim() !== "");
  ...
  const commonTextItems = textItems.filter(
    (item:any) => item.fontName === commonFontName && item.height === commonHeight
  );
