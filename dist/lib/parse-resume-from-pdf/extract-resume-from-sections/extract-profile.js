import { isBold, hasNumber, hasComma, hasLetter, hasLetterAndIsAllUpperCase, } from "../../parse-resume-from-pdf/extract-resume-from-sections/lib/common-features.js";
...
// Name
export const matchOnlyLetterSpaceOrPeriod = (item) => item.text.match(/^[a-zA-Z\s\.]+$/);
// Email
...
export const matchPhone = (item) => item.text.match(/\(?\d{3}\)?[\s-]?\d{3}[\s-]?\d{4}/);
const hasParenthesis = (item) => /\([0-9]+\)/.test(item.text);
// Location
// Simple location regex that matches "<City>, <ST>"
export const matchCityAndState = (item) => item.text.match(/[A-Z][a-zA-Z\s]+, [A-Z]{2}/);
// Url
